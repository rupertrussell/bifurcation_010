...
int bifurcationWidth = (8192/4)*4;
int bifurcationHeight = (128/4)*2;
float bifurcationPowers[];
int specificLine = 0;
int count, oldCount;

void setup() {
  size(3249, 2048); // was size(1620,1024);
  colorMode(HSB);
...
void draw() {
  ...
  for (int j = 0; j < bifurcationHeight; j++) {
    ...
    for (int i = 0; i < bifurcationWidth; i++) {
      point(10+map(i, 0, bifurcationWidth, 0, 3200), height*(1-(float)iteration[i][j]));
